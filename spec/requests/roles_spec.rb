...
RSpec.describe 'Roles API', type: :request do
  ...
  describe 'GET /roles/:id' do
    ...
    context 'when the record does not exist' do
      ...
      it 'returns a not found message' do
        expect(response.body).to match(/Couldn't find Role with 'id'=100/)
      end
    end
  end
  ...
  describe 'POST /roles' do
    # valid payload
    let(:valid_attributes) { { description: 'Superuser' } }

    context 'when the request is valid' do
      before { post '/roles', params: valid_attributes }

      it 'creates a role' do
        expect(json['description']).to eq('Superuser')
      end
  ...
  describe 'PUT /roles/:id' do
    let(:valid_attributes) { { description: 'Manager' } }
